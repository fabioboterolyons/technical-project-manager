    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):# TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            eval_prompt = (
                ...
                "Example: yes, this does not achieve the expectations because does not pass the unit tests"
            )
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {'role': 'system', 'content': "you are an evaluator agent and you must evaluate based on the evaluation criteria"},
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {'role': 'system', 'content': 'you are an step generator and you want to provide a list of instructions based in the evaluation description'},
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {'role': 'system', 'content': f"KNOWLEDGE:{self.knowledge} \n you are an action planner and you must analyze user prompt and split in single steps using your KNOWLEDGE, only return the steps"},
                {'role': 'user', 'content': f"split into steps: {prompt}"}
            ]
        )
